    ...
    const fetchInsights = async () => {
      ...
      try {
        ...
        const [rawRes, entityRes, todoRes] = await Promise.all([
          ...
          supabase.from("entity_ledger").select("name, trust_signal"),
        ...
        setData({
          ...
          total_entities: new Set((entities as { trust_signal: string; name?: string }[]).map((e) => (e.name ?? "").replace(/\s*\([^)]*\)\s*$/, "").toLowerCase().trim())).size,
