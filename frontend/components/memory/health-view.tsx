...
import { getHealthLogs, logHealth, analyzeHealth } from "@/lib/api-client";
import type { HealthLogRequestBody } from "@friday/shared";
...
export function HealthView({ isConfigured }: HealthViewProps) {
  ...
  const save = async (type: "sleep" | "steps" | "body") => {
    ...
    try {
      const today = todayStr();
      const payload: HealthLogRequestBody = { metric_type: type, log_date: today };
      if (type === "sleep") {
            ...
            {analysis && (
              <>
                ...
                <div className="rounded-xl bg-primary/8 border border-primary/15 p-3">
                  <p className="text-xs text-foreground/80 leading-relaxed italic">&ldquo;{analysis.week_summary}&rdquo;</p>
                </div>
        ...
        <div className="rounded-2xl glass-card p-4">
          ...
          <div className="space-y-1.5">
            {bodyLogs.slice(0, 6).map(log => (
              <div key={log.id ?? log.log_date} className="flex items-center justify-between text-xs">
...
function Panel({ open, onToggle, icon: Icon, iconColor, title, subtitle, children }: {
