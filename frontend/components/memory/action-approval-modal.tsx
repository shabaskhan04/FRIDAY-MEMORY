...
import { executeCommand, denyCommand } from "@/lib/api-client";
...
function CommandCard({
  ...
  return (
    <motion.div
      ...
      className={cn("rounded-2xl border p-4 transition-all", meta.bg, meta.border)}
    >
...
export function ActionApprovalModal() {
  ...
  // NOTE: Reading pending_commands directly via Supabase anon key is fine
  // because this is a single-user app and the realtime channel needs it.
  // Execute/deny go through the backend API (executeCommand / denyCommand).
  useEffect(() => {
