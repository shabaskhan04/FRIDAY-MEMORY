...
import { searchMemories, askFriday } from "@/lib/api-client";
...
export function MemoryTab({ isConfigured }: MemoryTabProps) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        const payload = await searchMemories({ query, limit: 50 });
        setSemanticResults((payload.memories ?? []) as RawLedger[]);
      } catch (err) {
  ...
  const handleAskFriday = async () => {
    ...
    try {
      const payload = await askFriday({ query });
      setAskResult(payload as AskResponse);
      setAskMode("done");
    } catch (err) {
      setAskResult({
