...
export function Header({ isOnline = true, pendingTodoCount = 0 }: HeaderProps) {
  ...
  const fetchWeeklySummary = useCallback(async () => {
    ...
    try {
      ...
      const data = (await res.json()) as WeeklySummary & { error?: string };
      if (!res.ok) throw new Error(data.error ?? "Failed to load weekly summary");
                ...
                <>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      { label: "People", value: weeklySummary.people_count },
                      { label: "Pending", value: weeklySummary.pending_todos },
                    ].map((s) => (
